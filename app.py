...
import json
#----------------------------------------------------------------------------#
# App Config. Connecting to database
#----------------------------------------------------------------------------#
...
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgres://postgres:123654789@localhost:5432/fyur'
...
class Venue(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(),nullable = False)
    city = db.Column(db.String(120), default = 'Uncategorized',nullable=False)
    state = db.Column(db.String(120), default = 'Uncategorized',nullable=False)
    address = db.Column(db.String(120), default = 'Uncategorized',nullable=False)
...
def index():
  data = dict()
  try:
    artists = Artist.query.order_by('id').all()[:10]
    venues = Venue.query.order_by('id').all()[:10]
    artists_data = []
    venues_data = []
    for artist,venue in zip(artists,venues):
      artists_data.append({
        'id': artist.id,
        'name': artist.name,
        'state': artist.state,
...
def venues():
  ...
  #error = False # for error tracking purposes
  ...
  print(exc_info())
  return render_template('pages/venues.html', data=av_ob_list) 
...
def search_venues():
  ...
  try:
    ...
    response['query_results'] = data
  except:
    error = True 
    print(exc_info())
  finally:
    # return the data with match
    size = len(response['query_results'])
    return render_template('pages/search_venues.html', response=response, search_term=request.form['search_term'],size = size)
...
def delete_venue(venue_id):
  error = False # for tracking errors
  # TODO: Complete this endpoint for taking a venue_id, and using
  to_delete = Venue.query.filter_by(id = venue_id).all()[0]
  #try:
...
def artists():
  ...
  try:
    ...
    if not error:
      ...
      render_template('errors/505.html')
...
def search_artists():
  ...
  for res in ar_res:
    to_show_as_results.append({
      'id':res.id,
      'name':res.name,
      'num_upcoming_shows': Show.query.filter(Show.artist_id == res.id, Show.startTime > datetime.now()).count()
    })
  result = {
    'count': len(ar_res),
...
def shows():
  ...
  # TODO: replace with real venues data.s 
  error = False
  ...
  size = len(res)
  return render_template('pages/shows.html', shows=res,size = size)
